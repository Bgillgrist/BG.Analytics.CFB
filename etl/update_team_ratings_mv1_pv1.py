...
# ────────────────────────────────────────────
# PERFORMANCE RATINGS (results-based)
# ────────────────────────────────────────────


def compute_performance_ratings(season: int, asof_date: str) -> pd.DataFrame:
    """Calculate performance ratings using completed games up to asof_date."""
    print("Connecting to database for performance ratings…")
    with psycopg.connect(PG_DSN) as conn:
...
def compute_market_ratings(season: int, asof_date: str) -> pd.DataFrame:
    """Calculate market-based ratings using spreads (past + near-future)."""
    print("Connecting to database for market ratings…")
